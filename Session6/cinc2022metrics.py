# -*- coding: utf-8 -*-
"""
Created on Wed Nov 23 14:09:42 2022
...
def compute_scores(murmur_labels,murmur_scalar_outputs, murmur_classes, outcome_labels,outcome_scalar_outputs, outcome_classes):
    # Evaluate the murmur model by comparing the labels and model outputs.
    
    murmur_binary_outputs = np.zeros((len(murmur_scalar_outputs),len(murmur_classes)), dtype=np.int_)
    idx = np.argmax(murmur_scalar_outputs,axis=1)
    for i in range(len(idx)):
        murmur_binary_outputs[i,idx[i]] = 1

    
    outcome_binary_outputs = np.zeros((len(outcome_scalar_outputs),len(outcome_classes)), dtype=np.int_)
    idx = np.argmax(outcome_scalar_outputs,axis=1)
    for i in range(len(idx)):
        outcome_binary_outputs[i,idx[i]] = 1
...
def print_scores(murmur_scores, outcome_scores):
    classes, auroc, auprc, auroc_classes, auprc_classes, f_measure, f_measure_classes, accuracy, accuracy_classes, weighted_accuracy, cost = murmur_scores
    murmur_output_string = 'AUROC,AUPRC,F-meas,Accuracy,Weighted Accuracy,Cost\n{:.3f},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}\n'.format(auroc, auprc, f_measure, accuracy, weighted_accuracy, cost)
